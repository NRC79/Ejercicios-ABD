...
# Parámetros de precisión y confianza
precision <- 0.01
confianza <- 0.99

# Calcular la varianza teórica de la distribución Beta
var_teorica <- (alpha * beta) / ((alpha + beta)^2 * (alpha + beta + 1))

# Calcular el número necesario de simulaciones
z <- qnorm((1 + confianza) / 2)  # Cuantil para nivel de confianza
n_simulaciones <- ceiling((z^2 * var_teorica) / (precision^2))
simulaciones <- rbeta(n_simulaciones, alpha, beta)

# Calcular media y varianza empíricas
media_empirica <- mean(simulaciones)
varianza_empirica <- var(simulaciones)

# Mostrar resultados
cat("Número de simulaciones necesarias:", n_simulaciones, "\n")
cat("Media empírica:", media_empirica, "\n")
cat("Varianza empírica:", varianza_empirica, "\n")
# Graficar la distribución resultante
hist(simulaciones, probability = TRUE, main = "Aproximación Monte Carlo de theta",
     xlab = expression(theta), col = "yellow", border = "red")
curve(dbeta(x, alpha, beta), add = TRUE, col = "green", lwd = 2)
```
...
theta_simulado <- rbeta(n_simulaciones, alpha, beta)
# Simular resultados binomiales para cada theta
resultados_binomiales <- rbinom(n_simulaciones, size = n_muestra, prob = theta_simulado)

# Representar la distribución predictiva previa
hist(resultados_binomiales, probability = TRUE, breaks = seq(0, n_muestra, by = 1),
     main = "Distribución Predictiva Previa",
     xlab = "Número de respuestas 'Sí'",
     col = "green", border = "darkgreen")
lines(density(resultados_binomiales, adjust = 1.5), col = "red", lwd = 2)

# Mostrar resumen de resultados
cat("Resumen de resultados binomiales:\n")
cat("Media predictiva previa:", mean(resultados_binomiales), "\n")
cat("Varianza predictiva previa:", var(resultados_binomiales), "\n")
```
...
```{r max-poisson}
max_poisson <- function(lambda, n) { # TODO: No borrar prototipo!
